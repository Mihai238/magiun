...
sealed case class MagiunMatrix(matrix: Matrix, columnNames: Seq[String] = Seq(), rowNames: Seq[String] = Seq()) {
  ...
  if (rowNames.nonEmpty) {
    require(rowNames.length == matrix.numRows, s"The number of row names ${rowNames.length} does not match the number of rows ${matrix.numRows}")
  }
