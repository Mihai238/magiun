...
class ColumnMetaDataComputerTest extends UnitTest {
  ...
  it should "try to guess column types for titanic dataset" in {
    ...
    columnsMeta(0).distributions.uniform should be(true)
    columnsMeta(0).distributions.exponential should be(false)
    ...
    columnsMeta(1).distributions.normal should be(false)
    columnsMeta(1).distributions.uniform should be(false)
    ...
    columnsMeta(5).distributions.uniform should be(false)
